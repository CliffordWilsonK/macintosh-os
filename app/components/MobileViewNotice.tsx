...
export default function MobileViewNotice() {
  ...
  return (
    <div className="min-[767px]:hidden min-h-[100dvh] flex flex-col">
      ...
      >
        <Link
          href="https://cliffordwilson.dev"
          target="_self"
          rel="noopener noreferrer"
          className="m-5 h-10 w-10 rounded-full grid place-items-center shadow-[0px_0px_5px] shadow-foreground/25 text-white bg-gradient-to-b from-blue-500 via-blue-600 to-blue-700"
        >
          <span className="drop-shadow-md inset-shadow text-sm font-semibold">
            CW
          </span>
        </Link>
      </motion.div>
      ...
      >
        <div className="mt-5 text-center px-10">
          <h1 className="text-2xl font-semibold leading-[1.2] mb-2">
            Not available on smaller screens 😓
          </h1>
          <p className="text-sm text-foreground/50">
            View full portfolio on the mobile version 👇
          </p>
        </div>
      </motion.div>
      ...
      <motion.div className="px-5">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-white">cliffordwilson.dev</span>
          </div>
      ...
      >
        © {new Date().getFullYear()} Clifford Wilson. All rights reserved.
      </motion.p>
    </div>
  );
}
