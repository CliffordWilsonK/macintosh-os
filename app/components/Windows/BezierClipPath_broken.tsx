...
const BezierClipPath: React.FC = () => {
  const [points, setPoints] = useState<BezierPoint[]>([
    { x: 0.4, y: 0.35, cp1x: 0.38, cp1y: 0.35, cp2x: 0.42, cp2y: 0.35, type: "smooth" },
    { x: 0.6, y: 0.35, cp1x: 0.58, cp1y: 0.35, cp2x: 0.6, cp2y: 0.4, type: "smooth" },
    { x: 0.6, y: 0.65, cp1x: 0.6, cp1y: 0.6, cp2x: 0.58, cp2y: 0.65, type: "smooth" },
    { x: 0.4, y: 0.65, cp1x: 0.42, cp1y: 0.65, cp2x: 0.4, cp2y: 0.6, type: "smooth" }
  ]);
  ...
  // Simple coordinate conversion without zoom/pan
  ...
  const getElementUnderCursor = useCallback((clientX: number, clientY: number) => {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      if (point.type === 'smooth' && point.cp1x !== undefined && point.cp1y !== undefined) {
      ...
      if (point.type === 'smooth' && point.cp2x !== undefined && point.cp2y !== undefined) {
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setPoints(prev => {
      ...
      if (dragState.handleType === 'point') {
        point.x = Math.max(0, Math.min(1, point.x + deltaX));
        point.y = Math.max(0, Math.min(1, point.y + deltaY));
        
        if (point.cp1x !== undefined) point.cp1x = Math.max(0, Math.min(1, point.cp1x + deltaX));
        if (point.cp1y !== undefined) point.cp1y = Math.max(0, Math.min(1, point.cp1y + deltaY));
        if (point.cp2x !== undefined) point.cp2x = Math.max(0, Math.min(1, point.cp2x + deltaX));
        if (point.cp2y !== undefined) point.cp2y = Math.max(0, Math.min(1, point.cp2y + deltaY));
  ...
  }, [dragState, getCanvasCoordinates]);
  ...
  const copyToClipboard = useCallback(() => {
    const pathData = points.map((point, index) => {
      if (index === 0) {
        return `M ${(point.x * 100).toFixed(1)}% ${(point.y * 100).toFixed(1)}%`;
      }
      
      const prevPoint = points[index - 1];
      const cp1x = prevPoint.cp2x !== undefined ? prevPoint.cp2x : prevPoint.x;
      const cp1y = prevPoint.cp2y !== undefined ? prevPoint.cp2y : prevPoint.y;
      const cp2x = point.cp1x !== undefined ? point.cp1x : point.x;
      const cp2y = point.cp1y !== undefined ? point.cp1y : point.y;
      
      return `C ${(cp1x * 100).toFixed(1)}% ${(cp1y * 100).toFixed(1)}%, ${(cp2x * 100).toFixed(1)}% ${(cp2y * 100).toFixed(1)}%, ${(point.x * 100).toFixed(1)}% ${(point.y * 100).toFixed(1)}%`;
    }).join(' ');
    
    const css = `clip-path: path('${pathData} Z');`;
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    if (dragState.isDragging) {
      ...
      const element = getElementUnderCursor(e.clientX, e.clientY);
      if (element.type === 'node' && element.index !== undefined) {
        setHoveredPoint(element.index);
      } else {
        setHoveredPoint(null);
      }
    }
  }, [dragState.isDragging, handleMouseMove, getElementUnderCursor]);
  ...
  return (
    <div className="h-full bg-gradient-to-br from-slate-50 to-slate-100 relative">
      ...
      <button
        ...
        className={`fixed top-6 right-6 z-10 p-3 rounded-xl shadow-lg border transition-all duration-200 ${
          copied
      ...
      <button
        ...
        className={`fixed top-6 left-6 z-10 p-3 rounded-xl shadow-lg border transition-all duration-200 ${
          showGrid
      ...
      <div className="h-full relative overflow-hidden">
        ...
        {/* Instructions */}
        <div className="absolute pointer-events-none bottom-6 left-1/2 transform -translate-x-1/2 bg-white/70 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200 px-4 py-2">
...
export default BezierClipPath;
  ...
  const getCanvasCoordinates = useCallback((clientX: number, clientY: number, applySuggestions = true, isControlPoint = false, anchorPoint?: BezierPoint) => {
    ...
    if (applySuggestions) {
      ...
      console.log('✨ Applied suggestions:', { original: { x, y }, suggested: suggestions });
      return { x: suggestions.x, y: suggestions.y };
    }
    
    setSnapGuides({});
    return { x, y };
  }, [pan, zoom, getSnapSuggestions]);
  ...
  const getElementUnderCursor = useCallback((clientX: number, clientY: number) => {
    ...
    // Calculate hit radius in normalized coordinates
    const hitRadius = 12 / Math.min(rect.width, rect.height);
...
{{ ... }}
      ctx.moveTo(visibleLeft * width, pixelY);
      ctx.lineTo(visibleRight * width, pixelY);
      ctx.stroke();
    }
    ...
    if (snapGuides.y !== undefined) {
      ...
      ctx.setLineDash([4 / zoom, 4 / zoom]);
      ctx.beginPath();
      ctx.moveTo(visibleLeft * width, snapGuides.y * height);
      ctx.lineTo(visibleRight * width, snapGuides.y * height);
      ctx.stroke();
    }
    
    ctx.restore();
  }, [showGrid, snapGuides, zoom, pan]);
  ...
  const zoomIn = useCallback(() => {
    ...
    const newZoom = Math.min(5, zoom * 1.005);
  ...
  const zoomOut = useCallback(() => {
    ...
    const newZoom = Math.max(0.1, zoom * 0.995);
    const newPanX = centerX - worldX * newZoom;
    const newPanY = centerY - worldY * newZoom;
  ...
  const resetZoom = useCallback(() => {
    ...
    if (canvasRef.current) {
      const rect = canvasRef.current.getBoundingClientRect();
      setPan({
        x: rect.width / 2 - 0.5 * rect.width,
        y: rect.height / 2 - 0.5 * rect.height
      });
    }
  }, []);
  ...
  const centerShape = useCallback(() => {
    ...
    setPan({
      x: canvasCenterX - centerX * zoom * rect.width,
      y: canvasCenterY - centerY * zoom * rect.height
    });
  }, [points, zoom]);
  ...
  useEffect(() => {
    if (history.length === 0 && points.length > 0) {
      ...
      // Center the edit area on initial load
    }
  ...
  }, [isSpacePressed, isShiftPressed, undo, redo, zoomIn, zoomOut, resetZoom, centerShape]);

  useEffect(() => {
    drawCanvas();
  }, [drawCanvas, points, showGrid, zoom, pan, snapGuides, hoveredPoint, suggestionNode]);
  ...
  return (
    <div className="h-full bg-gradient-to-br from-slate-50 to-slate-100 relative">
      ...
      <button
        ...
        className={`fixed top-20 left-6 z-10 p-3 rounded-xl shadow-lg border transition-all duration-200 ${
          snapEnabled
      ...
      {(() => {
        ...
        return displayPoint !== null && (
          <div 
            className={`fixed z-20 backdrop-blur-sm rounded-xl shadow-xl border p-3 pointer-events-auto ${
              selectedPoint !== null ? 'bg-blue-50/95 border-blue-200' : 'bg-white/95 border-slate-200'
            }`}
            style={{
              left: Math.min(window.innerWidth - 200, (points[displayPoint].x * 800 * zoom + pan.x + 50)),
              top: Math.max(20, (points[displayPoint].y * 600 * zoom + pan.y - 50))
            }}
          >
      ...
      >
        ...
        <div className="absolute pointer-events-none bottom-6 left-1/2 transform -translate-x-1/2 bg-white/70 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200 px-4 py-2">
          ...
          <div className="text-xs text-slate-500 flex items-center gap-4 mt-1 pt-1 border-t border-slate-200">
            <span>• C Center shape</span>
          </div>
        </div>
      </div>
    </div>
