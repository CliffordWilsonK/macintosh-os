...
export default function Terminal() {
  ...
  const getOutputForCommand = (cmd: string): string | undefined => {
    ...
    if (normalizedCmd === "whoami") return "Clifford Wilson";
    if (normalizedCmd === "date") return new Date().toString();
    if (normalizedCmd === "pwd") return "/Users/clifford";
    if (normalizedCmd === "hostname") return "MacBook-Pro.local";
    if (normalizedCmd === "uname") return "Techcosmocrat";
    if (normalizedCmd === "who")
      return `Clifford Wilson  console  ${new Date().toLocaleString("en-US", {
        month: "short",
        day: "2-digit",
        hour: "2-digit",
        minute: "2-digit",
    ...
    if (normalizedCmd === "uname -a")
      return "Techcosmocrat MacBook-Pro.local 23.4.0 Techcosmocrat Kernel Version 23.4.0: ... x86_64";
    if (normalizedCmd === "uname -s") return "Techcosmocrat";
    ...
    if (normalizedCmd === "ls -l" || normalizedCmd === "ls -la") {
      return launchpadApps
        .map(
          (app, i) =>
            `drwxr-xr-x  2 clifford  staff  ${i * 100 + 64}  May  7 09:${
              (i * 10) % 60
            }  ${app.title}`
        )
        .join("\n");
    ...
    if (normalizedCmd.startsWith("touch "))
      return "Sorry you do not have permission to create a file";
    if (normalizedCmd.startsWith("mkdir "))
      return "Sorry you do not have permission to create a directory";
    if (normalizedCmd.startsWith("rm "))
      return "Sorry you do not have permission to delete a file";
    if (normalizedCmd.startsWith("rmdir "))
      return "Sorry you do not have permission to delete a directory";
    ...
    if (normalizedCmd === "top") {
      return (
        "Processes: 342 total, 2 running, 340 sleeping, 1514 threads\n" +
        ...
        "1012   node         5.1  00:10:12 15    2    37    145M   clifford\n" +
        "1001   Google Chrome 3.2 00:04:32 32    1    89    485M   clifford\n" +
        "1045   Terminal     0.7  00:01:23 5     1    31    28M    clifford\n" +
        "988    Finder       0.5  00:03:13 7     2    42    68M    clifford"
      );
    }
    if (normalizedCmd === "ps")
    ...
    if (normalizedCmd === "ps aux") {
      return (
        ...
        "clifford    1012   5.1  1.8  4528236  297392   ??  S    09:00   0:10.12 node\n" +
        "clifford    1001   3.2  5.9  5726344  983812   ??  S    08:45   0:04.32 Google Chrome\n" +
        "clifford    1045   0.7  0.3  3156892   51284   ??  S    09:15   0:01.23 Terminal"
      );
    }
    ...
    if (normalizedCmd === "du -sh *")
      return "20M  Documents\n50M  Downloads\n10M  Projects\n5M   Applications\n2M   Music\n15M  Pictures";
    ...
    if (
      normalizedCmd === "python --version" ||
      normalizedCmd === "python3 --version"
    )
      return "Python 3.11.4";

    // Advanced system utilities (simplified output)
    if (normalizedCmd === "netstat -an")
      return "Active Internet connections (including servers)\nProto Recv-Q Send-Q  Local Address          Foreign Address        (state)\ntcp4       0      0  127.0.0.1.54932       127.0.0.1.8080        ESTABLISHED";
    if (normalizedCmd === "lsof")
      return "COMMAND     PID   USER   FD   TYPE DEVICE SIZE/OFF NODE NAME\nnode      1012 clifford  cwd    DIR  1,4    6144    2 /Users/clifford\nChrome    1001 clifford  txt    REG  1,4  8392704    3 /Applications/Google Chrome.app";
    ...
    >
      {lines.map((line, idx) => (
        <div key={idx} className="mb-1">
          ...
          >
            macbook@Techcosmocrat's-Machine ~ %
          </span>{" "}
          {line.command}
          {line.output && (
            <div
